...
/*protected $id_bol;
    ...
    public function valEstadoBol(){
        $con = conexion();
        $jsondata = array();
        $sql = "SELECT `EST_LOG_BOL` FROM `boletas` WHERE `ID_BOL`=".$this->id_bol;
        $this->total_bol = $sql;
        if ($result = $con->query($sql)) {
            if( $result->num_rows == 1 ) {
                while( $row = $result->fetch_object() ) {
    ...
    public function confEnv(){
        $con = conexion(); 
        $sql = "UPDATE `boletas` SET `EST_LOG_BOL`= 2 WHERE `ID_BOL`=".$this->id_bol." ";
        if ($result = $con->query($sql)) {
            $con->close();
            return 1;
        }else{
